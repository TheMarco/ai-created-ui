...
Dialog, Modal, and ConfirmDialog share one internal header and close-control pattern. Headers use 12px vertical padding, while the 44x44px close target is positioned independently so it does not make title-only headers taller. The centered 24px icon sits 16px from the panel's top and logical end. Hover strengthens the icon without drawing a border; keyboard focus retains the standard visible outline.
