...
`@ai-created/ui` is a private, raw TypeScript source package used by ai-created.com and Human, Actually. Both consumers install it from `github:TheMarco/ai-created-ui` and their lockfiles resolve an exact commit. The repository had a `1.0.0` package version but no release tags, changelog, release checks, or consumer update automation. Public npm publication is not a current product need.

## Decision

Keep GitHub distribution. Mark the package private to block accidental registry publication. Use stable SemVer in `package.json`, a matching annotated `vX.Y.Z` tag, dated human-reviewed changelog notes, and an automated GitHub Release after the complete quality gate passes.
...
- `private: true` makes the documented distribution boundary enforceable.
...
- Renovate must be installed with access to the private design-system and consumer repositories, and each consumer must copy the reviewed configuration template.
