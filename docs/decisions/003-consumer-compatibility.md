...
Status: Superseded in part by decision 004. Consumer-local compatibility pull requests remain current; private dependency authentication does not.
...
- Public HTTPS Git tags require no cross-repository read token. Workflows still install with scripts disabled before running lifecycle scripts and validation.
