        ...
        blocks: [
          {
            ...
            steps: [
              ...
              { title: 'Propose', owner: 'Consumer updater', output: 'An optional package.json and package-lock.json pull request', gate: 'Both files resolve the same immutable release tag.' },
              { title: 'Validate', owner: 'Consumer CI', output: 'Consumer-owned typecheck, lint, tests, policy checks, accessibility checks, and production build', gate: 'Applicable compatibility checks pass; a provider preview does not substitute for them.' },
              ...
              { title: 'Deploy and verify', owner: 'Consumer owner', output: 'Normal deployment plus an affected-workflow smoke test', gate: 'The deployed consumer behaves as reviewed.' },
              { title: 'Monitor', owner: 'Consumer owner', output: 'Optional scheduled freshness signal', gate: 'Monitoring reports staleness without changing the application.' },
            ],
          },
          {
            type: 'table',
            title: 'Choose an adoption path',
            columns: ['Path', 'Consumer choice', 'Expected result'],
            rows: [
          ...
          },
          {
            ...
            items: [
              { title: 'No registration required', description: 'The design-system repository does not need to know which applications consume it. Teams may maintain their own inventory when useful.' },
              { title: 'Compatibility is consumer-owned', description: 'Each application defines the commands and environments that prove the release works with its stack.' },
              { title: 'Provider checks are separate', description: 'Deployment previews help review the product but do not replace consumer-owned compatibility checks.' },
              { title: 'Completion includes production', description: 'A green pull request is not the endpoint. Merge manually, confirm deployment, and verify the affected product workflow.' },
              { title: 'Monitoring reports rather than mutates', description: 'The optional currency check exposes a stale tag. It does not open, merge, deploy, or verify an update.' },
      ...
      },
      {
        ...
        blocks: [
          ...
          },
          {
            ...
            items: [
              { title: 'Agents overview', description: 'Product-level explanation of how agents query the contract, what validation rejects, and how exceptions reach human review.', href: '/agents', action: 'Open page' },
              { title: 'AI agent integration guide', description: 'Consumer setup, required gates, repository enforcement, and adapter guidance.', href: 'https://github.com/TheMarco/ai-created-ui/blob/main/docs/agent-integration.md', action: 'Read guide', external: true },
            ],
          },
        ],
      },
